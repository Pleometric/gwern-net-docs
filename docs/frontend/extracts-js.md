...
extracts.js is the orchestration layer for gwern.net's popup/popover system. It sits between content.js (which knows *what* to show) and popups.js/popovers.js (which know *how* to show it), coordinating the entire extract display pipeline.

The module operates in two modes: popups (desktop) or popovers (mobile), selected automatically at setup based on viewport size and device detection. This single codebase handles both paradigms through a provider abstraction: `Extracts.popFrameProvider` points to either the `Popups` or `Popovers` object, and type-suffixed function names allow mode-specific behavior where needed.
...
Tears down the extract system. Used when user disables popups/popovers.
...
**Called by:** `preparePopup`, `preparePopover`, pop-frame content rewrite paths
...
preparePopoverTarget
        ↓
popFrameProvider.addTarget() → Registers with Popups or Popovers
        ↓
[user triggers popup/popover]
        ↓
preparePopup/preparePopover  → Main entry point from provider
        ↓
...
The `popFrameProvider` abstraction allows identical code paths for popups and popovers:
...
```javascript
let mobileMode = (
    localStorage.getItem("extracts-force-popovers") == "true"
    || GW.isMobile()
...
| `extract-popovers-disabled` | Disables popovers when "true" |
| `extracts-force-popovers` | Forces popover mode even on desktop |
...
| `Popups.didLoad` / `Popovers.didLoad` | Deferred setup if provider not ready |
...
- `Extracts.popFrameProvider` — Either `Popups` or `Popovers` object
- `Extracts.popFrameProviderName` — "Popups" or "Popovers" string
