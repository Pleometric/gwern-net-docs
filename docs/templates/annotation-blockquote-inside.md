...
Template for rendering annotations where the abstract is placed inside a blockquote within the title paragraph.

## Overview

This template produces a specialized annotation layout used when the annotation content needs to be rendered with the abstract contained within a `<blockquote>` element that is nested inside the title's paragraph structure. This creates a visual hierarchy where the title and metadata flow directly into the abstract without visual separation.

The template is part of gwern.net's annotation system, which provides rich metadata and abstracts for external links. The "blockquote-inside" variant is used when the semantic structure calls for the abstract to be subordinate to and contained within the title element's scope, creating a tighter visual and semantic coupling.
...
The template generates a nested structure:
